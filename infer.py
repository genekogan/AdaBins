...
class InferenceHelper:
    def __init__(self, dataset='nyu', device='cuda:0', pretrained_folder="./pretrained"):
        ...
        if dataset == 'nyu':
            ...
            pretrained_path = f"{pretrained_folder}/AdaBins_nyu.pt"
        elif dataset == 'kitti':
            self.min_depth = 1e-3
            self.max_depth = 80
            self.saving_factor = 256
            model = UnetAdaptiveBins.build(n_bins=256, min_val=self.min_depth, max_val=self.max_depth)
            pretrained_path = f"{pretrained_folder}/AdaBins_kitti.pt"
        else:
